...
def _encode_and_add_eos(line, subtokenizer):
  ...
  return subtokenizer.encode(line) + [EOS_ID]
...
def translate_file(model,
                   ...
                   src_subtokenizer,
                   tgt_subtokenizer,
                   input_file,  # bleu source file
                   output_file=None,  # bleu target file
  ...
  def input_generator():
    ...
    for i in range(num_decode_batches):
      ...
      lines = [_encode_and_add_eos(l, src_subtokenizer) for l in lines]
  ...
  for i, text in enumerate(input_generator()):
    ...
    for j in range(length):
      if j + i * batch_size < total_samples:
        translation = _trim_and_decode(val_outputs[j], tgt_subtokenizer)
...
def translate_from_text(model, src_subtokenizer, tgt_subtokenizer, txt):
  encoded_txt = _encode_and_add_eos(txt, src_subtokenizer)
  result = model.predict(encoded_txt)
  outputs = result["outputs"]
  logging.info("Original: \"%s\"", txt)
  translate_from_input(outputs, tgt_subtokenizer)
