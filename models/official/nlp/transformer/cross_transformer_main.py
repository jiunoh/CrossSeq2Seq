...
def evaluate_and_log_bleu(model,
  ...
  Returns:
    ...
    cased_score: A float, the case sensitive BLEU score.
  """
  src_subtokenizer = tokenizer.Subtokenizer(src_vocab_file)
  tgt_subtokenizer = tokenizer.Subtokenizer(tgt_vocab_file)  

  uncased_score, cased_score = translate_and_compute_bleu(model, params, src_subtokenizer, tgt_subtokenizer, bleu_source, bleu_ref, distribution_strategy)
...
class TransformerTask(object):
  ...
  def __init__(self, flags_obj):
    ...
    params["max_io_parallelism"] = (flags_obj.num_parallel_calls or tf.data.experimental.AUTOTUNE)
  ...
  def train(self):
    ...
    with distribution_utils.get_strategy_scope(self.distribution_strategy):
      model = cross_transformer.create_model(params, is_train=True, has_feature_layer=has_feature_layer)
      ...
      if latest_checkpoint:
        self._load_weights_if_possible(model, latest_checkpoint)

      emb = model.layers[2].input_embedding_layer
      enc = model.layers[2].encoder_stack
      dec = model.layers[2].decoder_stack
      tar_em = model.layers[2].target_embedding_softmax_layer

      emb_ckpt_obj = tf.train.Checkpoint(input_embedding_layer=emb)
      encoder_ckpt_obj = tf.train.Checkpoint(encoder_stack=enc)
      decoder_ckpt_obj = tf.train.Checkpoint(decoder_stack=dec)
      tar_ckpt_obj = tf.train.Checkpoint(target_embedding_softmax_layer=tar_em)

      emb_ckpt_path = 'pretrained_weights/pretrained_EN/pretrained_EN_input_embedding'
      enc_ckpt_path = 'pretrained_weights/pretrained_EN/pretrained_EN_encoder' 
      dec_ckpt_path = 'pretrained_weights/pretrained_DE/pretrained_DE_decoder'
      tar_ckpt_path = 'pretrained_weights/pretrained_DE/pretrained_DE_target_embedding'

      emb_ckpt_obj.restore(emb_ckpt_path).assert_consumed()
      encoder_ckpt_obj.restore(enc_ckpt_path).assert_consumed()
      decoder_ckpt_obj.restore(dec_ckpt_path).assert_consumed()
      tar_ckpt_obj.restore(tar_ckpt_path).assert_consumed()
    ...
    if self.use_tpu:
      ...
      train_ds = train_ds.map(map_data_fn, num_parallel_calls=tf.data.experimental.AUTOTUNE)
    if params["use_ctl"]:
      train_ds_iterator = iter(train_ds)

    val_ds = data_pipeline.eval_input_fn(params)
    map_data_fn = data_pipeline.map_data_for_transformer_fn
    val_ds = val_ds.map(map_data_fn, num_parallel_calls=tf.data.experimental.AUTOTUNE)
    ...
    def train_steps(iterator, steps):
      ...
      def _step_fn(inputs):
        ...
        with tf.GradientTape() as tape:
          logits = model([inputs, targets], training=True, has_feature_layer=has_feature_layer)
    ...
    train_summary_writer = tf.summary.create_file_writer(flags_obj.model_dir+'/bleu_log')
    ...
    while current_step < flags_obj.train_steps:
      ...
      logging.info("End train iteration at global step:{}\n".format(current_step))
      print("End train iteration at global step:{}\n\n".format(current_step))
      ...
      if (flags_obj.bleu_source and flags_obj.bleu_ref):
          ...
          with train_summary_writer.as_default():
            tf.summary.scalar('bleu', uncased_score, step=current_iteration+1)
  ...
  def eval(self):
    ...
    TARGET_VOCAB_FILE = self.flags_obj.data_dir + '/vocab.de_10K.10000'        
    ...
    with distribution_utils.get_strategy_scope(distribution_strategy):
      if not self.predict_model:
        self.predict_model = cross_transformer.create_model(self.params, is_train=False, has_feature_layer=has_feature_layer)
    ...
    return evaluate_and_log_bleu(
        ...
        self.flags_obj.bleu_ref, self.flags_obj.vocab_file, TARGET_VOCAB_FILE,
  ...
  def predict(self):
    ...
    with tf.name_scope("model"):
      model = cross_transformer.create_model(params, is_train=False, has_feature_layer=has_feature_layer)
    ...
    TARGET_VOCAB_FILE = self.flags_obj.data_dir + '/vocab.de_10K.10000'            
    tgt_subtokenizer = tokenizer.Subtokenizer(TARGET_VOCAB_FILE)
    ...
    for x, y in ds:
        try:
            ...
            x_arr = tf.make_ndarray(proto_tensor)
            proto_tensor = tf.make_tensor_proto(y)  # convert `tensor a` to a proto tensor
            y_arr = tf.make_ndarray(proto_tensor)
            for i, (x_row, y_row, val_output) in enumerate(zip(x_arr, y_arr, val_outputs)):
                print("input: ", translate.translate_from_input(x_row, subtokenizer))
                print("target: ", translate.translate_from_input(val_output, tgt_subtokenizer))
                print("ref: ", translate.translate_from_input(y_row, tgt_subtokenizer))
                print(
                    "--------------------------------------------------------------------------------------------------------------")
